...
trait ProjectCost
{
  ...
  private function ProjectCostByDates($qr)
  {
    $project_id = $qr['project_id'];
    $cost_from_date = $qr['fromDate'];
    $cost_to_date = $qr['toDate'];
    ...
    $cageQuery = Cage::where('project_id', $project_id)
                    ->where('start_date', '<=', $cost_from_date)
                    ->where('end_date', '<=', $cost_to_date)
                   ->orderBy('cage_id', 'asc')
                   ->get();
                   
    if(count($cageQuery) == 0){
        return false;
    }
    ...
    foreach ($cageQuery as $valx)
    {
      $cage_count = $cage_count + 1;
      $cage_id    = $valx->cage_id;
      //$start_date = $valx->start_date; // original cage start date
      //$end_date   = $valx->end_date;   // original cage end date
      
      $start_date = $cost_from_date;
      $end_date = $cost_to_date; 
  ...
  public function fetchCostByProjects()
  {
    ...
    foreach($projects as $project){
        ...
        $cost['projectId'] = $this->ProjectWiseCost($project->project_id);
    }
    //dd($cost['projectId'][1]);
    return $cost['projectId'][1];
  }
